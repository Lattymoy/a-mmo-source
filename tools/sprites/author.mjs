...
/* ── THE CAPE — authored poses, not a simulation.
   ...
   A simulated cape can only ever be filled polygons: no 1px specular run down a
   fold, no stepped highlight where cloth catches light, no hand-placed accent
   in a deep pleat. It reads as hand-PAINTED. Authoring the poses puts every
   pixel under control.

   Rasterized parametrically in polar space around the collar, which is what
   lets the 45-degree variants be AUTHORED at 45 degrees with correct pixel
   stepping instead of rotated at runtime — rotating pixel art resamples it.

   Six poses: rest / sway-left / sway-right, each cardinal and diagonal. The
   remaining directions come from 90-degree turns, which are pixel-exact. */
export const CAPE_SIZE = 63;
const CAPE_SPREAD = 82 * Math.PI / 180;   // half-angle — wide enough that the
                                          // cloth wraps past the body's sides
const CAPE_R0     = 8.0;                  // collar radius
const CAPE_LEN    = 27.0;                 // reach at the centre — the cloth
                                          // must be WIDER than the body or it
                                          // reads as a crown, not a cape
const CAPE_PLEATS = 5;
const CAPE_STEP   = 0.028;                // alternate pleats hang shorter. At the
                                          // cape's radius even a small fraction
                                          // is a visible step; 0.075 cut a deep
                                          // zigzag and it read as a starburst

function capePose(dirDeg, sway, skew){
  const N = CAPE_SIZE, c = N / 2;
  const g = blank(N, N);
  ...
  for(let y = 0; y < N; y++) for(let x = 0; x < N; x++){
    ...
    let a = Math.atan2(dx, dy) - d;
    a = ((a + Math.PI) % (2 * Math.PI)) - Math.PI;
    a += sway;
    if(Math.abs(a) > CAPE_SPREAD) continue;

    const u = (a + CAPE_SPREAD) / (2 * CAPE_SPREAD);      // across the cloth
    const p = Math.min(Math.floor(u * CAPE_PLEATS), CAPE_PLEATS - 1);
    const f = u * CAPE_PLEATS - p;                        // within the pleat

    // each pleat hangs to its own depth: a stepped hem, not a sawtooth
    let reach = CAPE_LEN * (1 - 0.10 * (Math.abs(a) / CAPE_SPREAD) ** 2)
                        * (1 + (p % 2 === 0 ? CAPE_STEP : -CAPE_STEP));
    reach += skew * a / CAPE_SPREAD * 4.0;
    if(r > reach) continue;

    // collar: the band the cloth gathers into, behind the head
    if(r < CAPE_R0){
      if(r > CAPE_R0 - 2.6) g[y][x] = f < 0.5 ? 'B' : 'A';
      continue;
    }
    // creases only once the cloth has opened out; at the collar they converge
    // into noise
    // creases start well out from the collar: near it they converge, and with
    // the head covering that region each pleat reads as a separate spike
    if(f < 0.10 && r > CAPE_R0 + 9.0) g[y][x] = 'K';
    else if(f < 0.20)                 g[y][x] = 'D';
    else if(f > 0.88)                 g[y][x] = 'A';
    else                              g[y][x] = p % 2 === 0 ? 'C' : 'B';
  }
  return { rows: rows(outlineExterior(g)), material: 'cloth' };
}

export const CAPE_POSES = () => ({
  rest0:   capePose(0,   0,     0),
  rest45:  capePose(45,  0,     0),
  swayL0:  capePose(0,   0.20,  0.35),
  swayL45: capePose(45,  0.20,  0.35),
  swayR0:  capePose(0,  -0.20, -0.35),
  swayR45: capePose(45, -0.20, -0.35),
});

export const MATERIALS = {
  cloth: {
    K: '#151013',   // keyline
    A: '#5A0A0E',   // deepest fold
    B: '#7E0F14',   // shadow
    C: '#C0161C',   // body
    D: '#E8323A',   // lit run
  },
